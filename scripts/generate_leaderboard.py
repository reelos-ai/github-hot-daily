...
def chinese_summary(name: str, summary: str, capabilities: list[str], relationship: str) -> str:
    if CHINESE_RE.search(summary):
        return summary
    focus = "、".join(capabilities[:3])
    direction = relationship or "开源能力"
    return f"{name} 是一个聚焦{focus}的开源项目，当前更适合作为「{direction}」方向的持续观察样本。"
...
def build_items(root: Path) -> tuple[list[dict], dict]:
    ...
    for name, repo_observations in observations.items():
        ...
        summary = (
            brief.get("summary")
            or report_summaries.get(name)
            or latest_repo.get("description")
            or "暂无可靠项目说明。"
        )
        ...
        provisional.append(
            {
                ...
                "summary": chinese_summary(name, summary, capabilities, relationship),
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .nav a:hover,.nav a.active{color:var(--blue);border-color:var(--line);background:rgba(255,255,255,.64)}
    .hero{
      display:grid;
    ...
    h1{margin:12px 0 14px;font-size:clamp(42px,7vw,78px);line-height:.96;letter-spacing:-.06em}
    ...
    .hero-meta b{display:block;font-size:24px;line-height:1.1}
    ...
    .field input:focus,.field select:focus{border-color:var(--blue);box-shadow:0 0 0 3px rgba(83,111,203,.1)}
    .result-head{display:flex;align-items:end;justify-content:space-between;gap:20px;padding:26px 0 12px}
    .result-head h2{margin:0;font-size:26px;letter-spacing:-.035em}
    ...
    .rank-row{
      display:grid;
      grid-template-columns:58px minmax(0,1fr) 116px 92px 110px;
      gap:18px;
      align-items:start;
      padding:22px 6px;
      border-bottom:1px solid var(--line);
    ...
    .rank-number{padding-top:2px;color:var(--blue);font-size:13px;font-weight:800}
    ...
    @media (max-width:840px){
      ...
      .metric.appearances{grid-column:2;grid-row:2}
      .metric.last-seen{grid-column:3;grid-row:2}
...
<body>
    ...
    <header class="topbar">
      <a class="brand" href="/" aria-label="ReelOS.ai">
        ...
        <span class="brand-copy">
          <span class="wordmark">Reel<b>OS</b>.ai</span>
      ...
      <nav class="nav" aria-label="主导航">
        ...
        <a href="/weekly/">周报</a>
        <a href="/monthly/">月报</a>
        <a class="active" href="/leaderboard/">总榜</a>
        <a href="/archive/">归档</a>
      </nav>
    </header>
    ...
    <main>
      <section class="hero">
        ...
        <div class="hero-meta" aria-label="数据概览">
          ...
          <div><b id="last-snapshot">--</b><span>更新至</span></div>
        </div>
      </section>

      <section class="method">
        <h2 class="mono">Ranking Method</h2>
        <p>总分 100：持续入榜 30%、增长信号 20%、战略评分 20%、近期热度 15%、最佳名次 10%、最近出现 5%。增长信号取“快照期间 Star 变化”和“单期最高增长”的较高值，避免重复累计周报数据。Star 只代表关注度，不等于采用度、生产可用性或商业价值。</p>
      </section>
      ...
      <section aria-live="polite">
        <header class="result-head">
          <div><h2>项目排名</h2><p id="result-status">正在读取榜单数据…</p></div>
  ...
  <script>
    (() => {
      const state = { items: [], meta: {}, q: "", language: "", sort: "score", page: 1, pageSize: 20 };
      ...
      function readUrl() {
        ...
        const requestedSize = Number.parseInt(params.get("pageSize") || "20", 10);
        state.pageSize = validPageSizes.has(requestedSize) ? requestedSize : 20;
      }
      ...
      function rowHtml(item, index) {
        ...
        return `<article class="rank-row" style="animation-delay:${Math.min(index * 22, 220)}ms">
          <span class="rank-number mono">#${String(item.rank).padStart(3, "0")}</span>
          <div class="repo-main">
            <div class="repo-title">
              <a href="${escapeHtml(item.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.full_name)}</a>
          ...
          <div class="metric last-seen"><b>+${formatNumber(item.growth_signal)}</b><span>增长信号 · ${escapeHtml(item.last_seen)}</span></div>
          <div class="metric score"><b>${item.rank_score}</b><span>综合得分</span></div>
        </article>`;
      }
      ...
      function render() {
        ...
        status.textContent = `共 ${items.length} 个项目 · 第 ${state.page} / ${totalPages} 页`;
      ...
      fetch("./data.json", { cache: "no-store" })
        ...
        .then((data) => {
          ...
          document.querySelector("#first-snapshot").textContent = state.meta.first_snapshot || "--";
          document.querySelector("#last-snapshot").textContent = state.meta.last_snapshot || "--";
          document.querySelector("#updated-at").textContent = `生成于 ${data.generated_at || "--"}`;
          render();
        })
        .catch((error) => {
          ranking.innerHTML = `<div class="error">榜单数据暂时无法读取：${escapeHtml(error.message)}</div>`;
...
def build_leaderboard(root: Path = ROOT) -> dict:
    ...
    payload = {
        ...
        "methodology": {
            ...
            "note": "Stars represent attention, not adoption or production readiness.",
        },
        "source": metadata,
        "items": items,
    }
