# GitHub Signal Intelligence

ReelOS GitHub 热榜情报站，发布 GitHub Trending 日报、周报、月报与往期归档。

## Site

- Production: https://gh.reelos.ai
- Cloudflare Pages: https://github-hot-daily.pages.dev
- GitHub: https://github.com/reelos-ai/github-hot-daily

## Structure

- `/` - 情报中心首页
- `/daily/` - 日报入口
- `/daily/2026-07-01/` - 当前日报
- `/weekly/` - 周报入口
- `/weekly/2026-06-29/` - 当前周报
- `/monthly/` - 月报入口
- `/monthly/2026-07/` - 当前月报
- `/archive/` - 往期归档
- `/reports.json` - 报告索引元数据

## Current Report

- HTML: `/monthly/2026-07/`
- Top 10 data: `/monthly-top10-2026-07.json`
- Raw trending data: `/monthly-trending-data-2026-07.json`
